<div>
    ...
    <div class="p-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        @if(!$pages->count() && !$search)
            <x-card>
                <x-slot:content>
                    <div class="max-w-lg mx-auto text-center">
                        <x-heroicon-o-flag class="mx-auto h-12 w-12 text-slate-400" />

                        <h3 class="mt-2 text-lg font-medium text-slate-900 dark:text-slate-200">
                            {{ __('Add pages to your store') }}
                        </h3>

                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                            {{ __('Write clear page titles and descriptions to improve your search engine optimization (SEO) and help customers find your website.') }}
                        </p>
        ...
        @else
                                                    ...
                                                    <div class="space-x-0.5">
                                                        ...
                                                        @if($pages->total() > $pages->count())
                                                            <button
                                                                ...
                                                                class="btn btn-link"
                                                            >
                                                                {{ $selectAll ? __('Clear selection') : __('Select all :count pages', ['count' => $pages->total()]) }}
                                                            </button>
                                                        @endif
                                                    </div>
                                                @endunless
                                            ...
                                            >
                                                <td class="relative w-12 px-6 sm:w-16 sm:px-8">
                                                    @if(in_array($page->id, $selected))
                                                        <div
                                                            class="absolute inset-y-0 left-0 w-0.5 bg-sky-500 dark:bg-sky-400"
                                                        ></div>
                                                    @endif
                                                    <x-input
                                                        wire:model="selected"
                                                        wire:key="checkbox-{{ $page->id }}"
                                                        type="checkbox"
                                                        value="{{ $page->id }}"
                                                        class="absolute left-4 top-1/2 -mt-2 h-4 w-4 !rounded !shadow-none sm:left-6"
                                                    />
                                                ...
                                                <td class="relative px-3 py-4 font-medium text-sm text-slate-900 text-left whitespace-nowrap dark:text-slate-200">
                                                    <a
                                                        href="{{ route('employee.pages.detail', $page) }}"
            ...
            <x-modal-alert wire:model="showDeleteConfirmationModal">
                ...
                <x-slot:content>
                    {{ trans_choice('Are you sure you want to delete :count page?|Are you sure you want to delete :count pages?', count($selected)) }}
    ...
    <form wire:submit.prevent="saveNewPage">
        <x-modal-dialog wire:model="addingNewPage">
            <x-slot:title>
                {{ __('Add new page') }}
            </x-slot:title>
            <x-slot:content>
                <div class="space-y-6">
                    <div>
                        <x-input-label
                            for="newPageTitleInput"
                            :value="__('Title')"
                        />
                        <x-input
                            wire:model.defer="newPage.title"
                            type="text"
                            id="newPageTitleInput"
                            class="block w-full mt-1 sm:text-sm"
                            placeholder="{{ __('Eg: About us, Sizing chart') }}"
                            autofocus
                        />
                        <x-input-error
                            for="newPage.title"
