<div>
    <form wire:submit.prevent="save">
        <x-modal-dialog wire:model.defer="isShown">
            <x-slot:title>
                {{ $this->taxZone->exists ? __('Edit zone') : __('Create zone') }}
            </x-slot:title>
            <x-slot:content>
                <div class="space-y-5">
                    <div>
                        ...
                        />
                        <x-input
                            wire:model.defer="taxZone.name"
                            type="text"
                            id="taxZoneName"
                            class="mt-1 block w-full sm:text-sm"
                        />
                        @if($errors->has('taxZone.name'))
                            <x-input-error
                                for="taxZone.name"
                                class="mt-2"
                            />
                            ...
                            <label class="relative flex items-start px-4 py-4 sm:px-6 hover:bg-slate-50 cursor-pointer dark:hover:bg-slate-700/25">
                                <div class="flex items-center h-5">
                                    <x-input
                                        ...
                                        :disabled="!$this->taxZone->exists && in_array($country['id'], $this->countriesInAnotherZones) || $this->taxZone->exists && in_array($country['id'], $this->countriesInAnotherZones) && !in_array($country['id'], $taxZone->countries->pluck('country_id')->toArray())"
                                    />
                                </div>
                                <div class="ml-3 flex flex-1 items-center justify-between text-sm">
                                    ...
                                    @if(!$this->taxZone->exists && in_array($country['id'], $this->countriesInAnotherZones) || $this->taxZone->exists && in_array($country['id'], $this->countriesInAnotherZones) && !in_array($country['id'], $taxZone->countries->pluck('country_id')->toArray()))
